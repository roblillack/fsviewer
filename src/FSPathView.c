...
FSCreatePathView(WMWidget* parent)
{
    ...
    WMCreateEventHandler(pvPtr->view, ExposureMask | StructureNotifyMask | ClientMessageMask, handleEvents, pvPtr);
...
void FSSetPathViewMaxVisibleColumns(FSPathView* pvPtr, int columns)
{
...
int FSGetPathViewNumberOfColumns(FSPathView* pvPtr)
{
...
void FSSetPathViewFillColumnProc(FSPathView* pvPtr, FSPathViewFillColumnProc* proc)
{
    pvPtr->fillColumn = proc;
}

int FSGetPathViewFirstVisibleColumn(FSPathView* pvPtr)
{
...
void FSSetPathViewAction(FSPathView* pvPtr, WMAction* action, void* clientData)
{
    pvPtr->action = action;
    pvPtr->clientData = clientData;
}

void FSSetPathViewDoubleAction(FSPathView* pvPtr, WMAction* action,
    void* clientData)
{
...
void FSSetPathViewHasScroller(FSPathView* pvPtr, int hasScroller)
{
...
void FSLoadPathViewColumnZero(FSPathView* pvPtr)
{
    if (!pvPtr->flags.loaded) {
        /* create column 0 */
        FSAddPathViewColumn(pvPtr);
...
int FSAddPathViewColumn(FSPathView* pvPtr)
{
    FSFileButton* list;
    FSFileButton** clist;
    char** tlist;

...
{
    if (!(info = FSCreateInfoPanel(app, title, msg))) {
        return;
    }
...
char* FSRunAppInputPanel(FSViewer* app, FileInfo* fileInfo, char* title)
{
    ...
    if (!(appInput = FSCreateAppInputPanel(app, fileInfo, title))) {
        return NULL;
    }
...
char* FSRunSelectIconPanel(WMWindow* owner, char* title, char* str)
{
    char* imgStr = NULL;
...
FSCreateSelectIconPanel(WMWindow* owner, char* title, char* str)
{
    ...
    selIcon->win = WMCreateWindowWithStyle(selIcon->scr, "selIcon",
        WMTitledWindowMask | WMClosableWindowMask | WMBorderlessWindowMask);
...
char* getSelectedFilename(FSSelectIconPanel* panel)
{
    WMListItem* listItem;
    FileInfo* fileInfo;
    char* filename;

...
FSCreateFileView(FSViewer* fsViewer, char* path, Bool primary)
{
    ...
    */
    ...
    if ((fView->size = XAllocSizeHints())) {
        ...
        fView->size->max_height = DisplayHeight(fView->dpy, DefaultScreen(fView->dpy));
...
FSFindFileIconWithFileInfo(FSFileView* fView, FileInfo* fileInfo)
{
    ...
    while (tmp) {
        ...
        if (strcmp(fileInfo->name, fI->name) == 0 && strcmp(fileInfo->path, fI->path) == 0) {
...
void FSSetFileViewMode(FSFileView* fView, FSFileViewType mode)
{
    ...
    switch (mode) {
    case Browser:
        FSSetFileBrowserMode(fView->fileBrowser, 0);
        break;
    case Icon:
        FSSetFileBrowserMode(fView->fileBrowser, 1);
        break;
    case List:
        FSSetFileBrowserMode(fView->fileBrowser, 2);
        break;
    default:
        FSSetFileBrowserMode(fView->fileBrowser, 0);
        break;

...
char* FSParseExecString(char* pathname, char* execStr, ...)
{
    va_list ap;
    char* str;
    char* buffer;
        ...
        if (event == AppExec) {
            ...
            if ((exec = FSGetStringForNameKey(extn, "viewer")) != NULL)
                ...
                */
                ...
                if (!strcmp("ascii", buf) || !strcmp("HTML", buf) || !strcmp("MAIL", buf)) {
                    exec = FSGetStringForNameKey("MAGICASCII", "exec");
                } else if (!strcmp("PBM", buf) || !strcmp("PGM", buf) || !strcmp("GIF", buf) || !strcmp("JPG", buf) || !strcmp("XPM", buf) || !strcmp("XBM", buf) || !strcmp("PPM", buf) || !strcmp("TIFF", buf)) {

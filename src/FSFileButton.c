...
FSCreateFileButton(WMWidget* parent)
{
     ...
     * action events
     */
    WMCreateEventHandler(bPtr->view, ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask,
...
radioPushObserver(void* observerData, WMNotification* notification)
{
    ...
    FSFileButton* pushedButton = (FSFileButton*)WMGetNotificationObject(notification);
...
void FSSetFileButtonPathname(FSFileButton* bPtr, char* pathname, int isBranch)
{
    CHECK_CLASS(bPtr, fileButtonWidgetClass);
...
void FSSetFileButtonAction(FSFileButton* bPtr, WMAction* action, void* clientData)
{
    CHECK_CLASS(bPtr, fileButtonWidgetClass);

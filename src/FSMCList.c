...
void FSResizeMCList(FSMCList* mPtr, unsigned int width, unsigned int height)
{
...
void FSScrollMCListToColumn(FSMCList* mPtr, int col, Bool updateScroller)
{
    FSScrollBrowserToColumn(mPtr->browser, col, updateScroller);
}

void FSSetMCListAction(FSMCList* mPtr, WMAction* action, void* clientData)
{
    FSSetBrowserAction(mPtr->browser, action, clientData);
}

void FSSetMCListDoubleAction(FSMCList* mPtr, WMAction* action, void* clientData)
{
